...
if (multiple):
    list_of_PIL_images = [];
    for imgPath in json.loads(data):
        img = Image.open(imgPath.split(':\n')[1]);
    ...
    if (':\n' in data):
        img = Image.open(data.split(':\n')[1]);
    else:
        img = Image.open(data);

    vector = img2vec.get_vec(img, tensor=True);

    print(np.array(vector).flatten().tolist());
    sys.stdout.flush();
